    ...
    >
      ...
      >
        <template slot-scope="scope">
          ...
          >
          <el-button
            v-if="scope.row.hedusername != null"
            size="mini"
            type="text"
            icon="el-icon-info"
          ...
          >
          <el-button
            v-if="scope.row.status==1 && scope.row.hedstatus == 1"
            size="mini"
            type="text"
            icon="el-icon-success"
            @click="Helpsuccess(scope.row)"
            v-hasPermi="['ma:help:remove']"
            >完成</el-button
          >
        </template>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      mylistHelp(this.queryParams).then((response) => {
